...
// Su Vercel /tmp è l'unica directory scrivibile
...
async function saveDebugImage(filename: string, buffer: Buffer): Promise<void> {
  ...
  try { await writeFile(path.join(generatedDir(), filename), buffer); } catch {}
}
...
async function cleanGeneratedSubject(inputBuffer: Buffer): Promise<Buffer> {
  try {
    ...
    console.error("[generate-visual] Cleaning failed:", error);
    return inputBuffer;
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    for (let i = 0; i < VARIANT_COUNT; i++) {
      console.log(`[generate-visual] Generating variant ${i+1}/${VARIANT_COUNT}`);
      ...
      console.log(`[generate-visual] Variant ${i+1} done: ${filename}`);
    ...
    return NextResponse.json({ variants: personUrls, personVariants: personUrls, model, size });
  } catch (error) {
